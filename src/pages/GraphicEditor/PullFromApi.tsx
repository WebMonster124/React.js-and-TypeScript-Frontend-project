  ...
  <SvgIcon {...props} viewBox="0 0 256 256">
    <defs>
      <radialGradient
        ...
        fy="-50%"
        r="100%"
        id="IconifyId-173deda66e0-a17b39-6">
    ...
    <path
      ...
      fill="url(#IconifyId-173deda66e0-a17b39-6)"
    />
  </SvgIcon>
);
...
interface Props extends StateProps, DispatchProps {
  graphic: Graphic;
  onPull(data: any): void;
}

const PullFromAPI = (props: Props) => {
  const classes = useStyles();
  const { t } = useTranslation();
  const { enqueueSnackbar } = useSnackbar();

  const [url, setUrl] = useState(props.graphic.dataApiEndpoint || '');
  const [loading, setLoading] = useState(url.length === 0);

  const handlePull = () => {
    logger.info('GraphicEditor/API/PullHandler');
    setLoading(true);
    logger.debug('Requesting the url', { url: url.trim() });
    axios
      .get(url.trim())
      .then((res) => {
        logger.debug('response succeeded', { data: res.data });
        setLoading(false);
        if (props.onPull) props.onPull(res.data);
        enqueueSnackbar('Data pulled', {
          variant: 'success',
        });
        logger.debug('Updating graphic object', { url: url.trim() });
        props.updateGraphic({
          graphicId: props.graphic.graphicId,
          body: {
            dataApiEndpoint: url.trim(),
          },
        });
      })
      .catch((e) => {
        logger.error('Error pulling data', { api: e });
        setLoading(false);
        enqueueSnackbar('Error pulling data', {
          variant: 'error',
        });
      });
  };
    ...
    <Paper className={classes.root}>
      <TextField
        ...
        onChange={(e) => {
          ...
          if (newUrl.trim().length && loading) setLoading(false);
          else if (newUrl.trim().length === 0) setLoading(true);
          setUrl(newUrl);
        }}
        label={t('graphic_version_editor.api_endpoint')}
        size="small"
        fullWidth
      ...
      <Button
        ...
        size="large"
        disabled={loading}>
        {t('graphic_version_editor.pull_data_from_api')}
      </Button>
      <IconButton
        component="a"
        href="https://input-tool-3-apiextensions.netlify.app"
        target="_blank"
        // variant="outlined"
        className={classes.button}>
        <NetlifyIcon />
...
export default connect(mapState, mapDispatch)(PullFromAPI);
